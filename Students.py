...
def CNNlayers():
    ...
    re1 = tf.reshape(pool5, [-1, 6 * 6 * 256])
...
for epoch in range(n_epoch):
    ...
    for x_train_a, y_train_a in minibatches(train_images_list, data.label, batch_size, shuffle=True):
        _,err,ac=sess.run([train_op,loss,acc], feed_dict={input: x_train_a, output: y_train_a})
    ...
    for x_val_a, y_val_a in minibatches(train_images_list, data.label, batch_size, shuffle=False):
        err, ac = sess.run([loss,acc], feed_dict={input: x_val_a, output: y_val_a})
